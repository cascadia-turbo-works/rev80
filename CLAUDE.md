...
| `sample.py` | `AcquisitionSettings` (derived samplerate/blocksize/nperseg/band from maxfreq/binsize; declared band; averaging; per-channel names, target units, amplitude modes, couplings, voltage ranges) + `VibeSample` (one channel's raw block, with cached PSD and filtered data) + `ChannelResult` (frozen display result) |
...
| `envelope.py` | Envelope (demodulation) analysis — `envelope_spectrum()` (band-pass → Hilbert magnitude → DC removal → amplitude spectrum) and `suggest_band()`. The band-pass, not the Hilbert transform, is the load-bearing step |
...
- `DataCollector.process_sample()` (not `VibeSample.process()`) produces the `ChannelResult`. Per frame: Welch PSD (one segment — `nperseg == blocksize`), optional averaging over N frames, five integration orders, spectrum truncated at `maxfreq`, peaks, overall, waveform, crest factor and kurtosis.
...
`VibeSensor.simulated()` returns a sensor with `is_simulation=True`. `VibeSensor.connect()` checks this flag and returns a `SimulatedSensor` instead of a hardware stream.
