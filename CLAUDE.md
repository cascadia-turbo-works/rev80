...
- `DataCollector.data_callback` appends the frame to a 32-frame ring cache (deque) and sets `new_frame_event`. The GUI's `poll_new_frames()` checks this event each render tick and displays the latest frame.
