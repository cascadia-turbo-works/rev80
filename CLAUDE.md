...
- **CI installers are driver-less by choice.** The release build passes `nodlls`, so no DLLs are bundled; `rev80.spec` bundles `drivers/` whether or not it holds DLLs and `_pico_loader.ensure_pico_dlls_loadable()` warns rather than raises, so nothing downstream needed changing — but the user must install PicoSDK themselves.
- **PicoSDK is nevertheless installed on the Windows runner, and must be.** `picosdk`'s `setup.py` probes for the native DLLs at *install* time: `ctypes.WinDLL(find_library(name))` with no SDK raises `TypeError` (its `except OSError` misses it, and `if not atleast1dll: exit(1)` would fail regardless), so `pip install` dies before any Rev80 code runs. On Linux the same line is `cdll.LoadLibrary(None)`, which is legal — which is why the ubuntu jobs install it happily and a Windows job cannot. Pre-installing the wrapper does not help; the dependency is a direct git URL, so pip re-clones and re-runs `setup.py` regardless. No reboot is required: the restart PicoSDK asks for registers the USB *kernel driver*, not the user-mode DLL the probe loads.
