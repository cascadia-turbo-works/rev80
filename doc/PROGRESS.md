...
| R46 | Sep 2026 | **Surface velocity from reflector size and duty cycle** — with reflector arc length L, measured duty d and shaft rate f, the reflector subtends d of a revolution, so circumference is L/d and surface velocity is v = f·L/d. The tape doubles as a shaft-diameter measurement, giving surface speed without anyone measuring the shaft — directly useful on rollers, belts and web handling. **Blocked on duty cycle, which is not currently captured:** `detect_edges` finds only the active edge, and `TachResult` carries no pulse width. Needs closing-edge detection in `tach.py`, a width field on `TachResult`, and a persistence decision (D-2 stores rising edge times only). Accuracy caveat to measure before shipping: an optical tach's finite spot width inflates the observed duty, which under-reads radius and velocity — worst on a small shaft with a wide spot. | 🔲 TODO |
