...
BUILTIN_SENSORS are always prepended to all() and cannot be modified or deleted.
"""
...
class ScopeSensorRegistry:
    ...
    Builtins are always present in all() but are never written to disk.
    User sensors are read from / written to the YAML file.
    """
    ...
    def all(self) -> list[ScopeSensor]:
        """Return builtins followed by user-defined sensors."""
        return list(BUILTIN_SENSORS) + self._load_user()
    ...
    def add(self, sensor: ScopeSensor) -> None:
        if sensor.id.startswith('builtin-'):
            raise ValueError('Cannot add a sensor with a builtin ID')
    ...
    def update(self, sensor: ScopeSensor) -> None:
        if sensor.id.startswith('builtin-'):
            raise ValueError('Cannot modify builtin sensors')
        user = self._load_user()
    ...
    def delete(self, sensor_id: str) -> None:
        if sensor_id.startswith('builtin-'):
            raise ValueError('Cannot delete builtin sensors')
        user = self._load_user()
